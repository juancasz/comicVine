...
const Details = ({hideBrowser}) => {
    ...
    useEffect(() => {
        hideBrowser()
        const url = `https://comicvine.gamespot.com/api/issue/4000-${idDetails}/?api_key=${api_key_comic}&format=json&field_list=image,character_credits,team_credits,location_credits,concept_credits`
        axios.get(proxy+url)
        .then(response => {
    ...
    },[idDetails,api_key_comic,hideBrowser])
